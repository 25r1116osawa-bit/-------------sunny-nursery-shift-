...
import { getWeekDays, formatDate, isWeekend, getShiftPosition } from './utils';
...
const App: React.FC = () => {
  ...
  const activeShifts = useMemo(() => {
    if (viewMode === 'WEEK') {
      ...
      return shifts.filter(s => s.date === activeDateStr);
    }
  }, [shifts, weekDays, activeDateStr, viewMode]);
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-[1600px] mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-gray-800 hidden sm:block">Sunny Nursery Shift</h1>
          </div>
      ...
      <main className="max-w-[1600px] mx-auto px-4 mt-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <Users size={16} />
            <span>表示人数: {filteredStaff.length}名</span>
          </div>
        </div>
                      ...
                      <div key={dateStr} className={`p-3 text-center border-r border-gray-200 last:border-0 ${holiday || weekend ? 'bg-orange-50' : ''}`}>
                        ...
                        <div className="mt-2 flex flex-col items-center gap-1">
                          <div className="flex items-center gap-1 text-[10px] font-medium text-gray-500">
                            正社員必要: 
                            <input 
                              type="number" min="0" className="w-8 border-b border-gray-300 bg-transparent text-center focus:outline-none"
                              value={getRequirementForDate(dateStr)}
                              onChange={(e) => setRequirementForDate(dateStr, parseInt(e.target.value) || 0)}
                            />
                          </div>
                          <div className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${staffing.isMet ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            {staffing.count} / {staffing.req}
                          </div>
                        </div>
                      </div>
                    );
                    ...
                    <div key={member.id} className="grid grid-cols-[200px_repeat(7,1fr)] group hover:bg-gray-50 transition-colors">
                      <div className="p-4 border-r border-gray-200 flex flex-col gap-1">
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={() => deleteStaff(member.id)} className="p-1 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={14} /></button>
                        </div>
                          ...
                          <div key={dateStr} className="p-2 border-r border-gray-100 last:border-0 relative h-20 flex flex-col items-center justify-center cursor-pointer" onClick={() => setShowShiftModal({ staff: member, date: dateStr })}>
                            {shift ? (
                              <div className="w-full h-12 rounded-lg flex flex-col items-center justify-center text-[11px] font-bold text-white shadow-sm" style={{ backgroundColor: member.color }}>
                                ...
                                <div className="h-0.5 w-4 bg-white/50 my-0.5"></div>
            ...
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <div className="flex items-center gap-4">
                ...
                {MOCK_HOLIDAYS.find(h => h.date === activeDateStr) && (
                  <span className="px-2 py-0.5 bg-red-100 text-red-700 text-xs font-bold rounded-full">
                    {MOCK_HOLIDAYS.find(h => h.date === activeDateStr)?.name}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2 text-sm font-medium text-gray-500">
                  正社員必要数: 
                  <input 
                    type="number" min="0" className="w-12 px-2 py-1 border border-gray-200 rounded bg-white text-center focus:outline-none focus:ring-2 focus:ring-blue-500"
                    value={getRequirementForDate(activeDateStr)}
                    onChange={(e) => setRequirementForDate(activeDateStr, parseInt(e.target.value) || 0)}
                  />
                </div>
                {(() => {
                  const staffing = getStaffingLevel(activeDateStr);
                  return (
                    <div className={`px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2 ${staffing.isMet ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                      {staffing.isMet ? '充足' : '不足'} : {staffing.count} / {staffing.req} 名
                    </div>
                  );
                })()}
              </div>
              ...
              <div className="min-w-[1000px] relative">
                ...
                <div className="grid grid-cols-[200px_1fr] bg-gray-50 border-b border-gray-200">
                  ...
                  <div className="relative h-12 flex">
                    {TIME_SLOTS.filter((_, i) => i % 2 === 0).map((hour, i) => (
                      <div key={hour} className="flex-1 border-l border-gray-200 text-[10px] text-gray-400 pl-1 pt-1">
                        {hour}
                      </div>
                    ))}
                  </div>
                ...
                <div className="divide-y divide-gray-100 relative">
                  ...
                  <div className="absolute inset-y-0 left-[200px] right-0 grid grid-cols-14 pointer-events-none">
                    {Array.from({ length: 14 }).map((_, i) => (
                      <div key={i} className="border-l border-gray-50 last:border-r h-full"></div>
                    ))}
                  </div>
                    ...
                    return (
                      <div key={member.id} className="grid grid-cols-[200px_1fr] min-h-[64px] group hover:bg-gray-50/50 transition-colors">
                        <div className="p-3 border-r border-gray-200 flex flex-col justify-center">
                          ...
                          <span className="text-[9px] text-gray-400 mt-0.5">{member.type}</span>
                        </div>
                        <div className="relative h-full flex items-center px-0 cursor-pointer" onClick={() => setShowShiftModal({ staff: member, date: activeDateStr })}>
                          {shift ? (
                            <div 
                              className="absolute h-10 rounded-lg flex items-center justify-between px-3 text-[10px] font-bold text-white shadow-md hover:brightness-105 transition-all z-10"
                              style={{ 
                            ...
                            >
                              ...
                              <span className="truncate">{shift.endTime}</span>
                            </div>
                          ) : (
                            <div className="w-full h-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                              <span className="text-gray-300 text-xs flex items-center gap-1 font-medium"><Plus size={14}/> シフト追加</span>
                            </div>
                          )}
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-blue-50/50">
              <h2 className="text-xl font-bold text-gray-800">職員の追加</h2>
            ...
            <form onSubmit={(e) => {
              ...
              addStaff(formData.get('name') as string, formData.get('type') as EmploymentType);
            }} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">名前</label>
                <input name="name" required className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all" placeholder="例: 大澤 樹" />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">雇用形態</label>
                <select name="type" className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all bg-white">
                  {Object.values(EmploymentType).map(type => <option key={type} value={type}>{type}</option>)}
                </select>
              </div>
              <div className="pt-4 flex gap-3">
                <button type="button" onClick={() => setShowStaffModal(false)} className="flex-1 px-4 py-3 border border-gray-200 rounded-xl font-bold text-gray-600 hover:bg-gray-50 transition-colors">キャンセル</button>
                <button type="submit" className="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-200 transition-all">追加する</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-gray-800">{showShiftModal.staff.name}</h2>
                <p className="text-sm text-gray-500">{showShiftModal.date} のシフト設定</p>
              </div>
              <button onClick={() => setShowShiftModal(null)} className="text-gray-400 hover:text-gray-600">
                <AlertCircle size={24} className="rotate-45" />
              </button>
            </div>
            <form onSubmit={(e) => {
              e.preventDefault();
              const formData = new FormData(e.currentTarget);
              upsertShift(showShiftModal.staff.id, showShiftModal.date, formData.get('start') as string, formData.get('end') as string);
            }} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">開始時間</label>
                  <select name="start" defaultValue={shifts.find(s => s.staffId === showShiftModal.staff.id && s.date === showShiftModal.date)?.startTime || "09:00"} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white">
                    {TIME_SLOTS.map(slot => <option key={slot} value={slot}>{slot}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">終了時間</label>
                  <select name="end" defaultValue={shifts.find(s => s.staffId === showShiftModal.staff.id && s.date === showShiftModal.date)?.endTime || "18:00"} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white">
                    {TIME_SLOTS.map(slot => <option key={slot} value={slot}>{slot}</option>)}
                  </select>
                </div>
              </div>
              <div className="pt-4 flex flex-col gap-3">
                <button type="submit" className="w-full px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-200">
                  {shifts.find(s => s.staffId === showShiftModal.staff.id && s.date === showShiftModal.date) ? '更新する' : '保存する'}
                </button>
                {shifts.find(s => s.staffId === showShiftModal.staff.id && s.date === showShiftModal.date) && (
                  <button type="button" onClick={() => deleteShift(showShiftModal.staff.id, showShiftModal.date)} className="w-full px-4 py-3 text-red-600 font-bold hover:bg-red-50 rounded-xl transition-colors flex items-center justify-center gap-2">
                    <Trash2 size={18} /> シフトを削除
                  </button>
                )}
